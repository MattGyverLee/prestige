  ...
  constructor(sources) {
    ...
    this.addMilestone = function(index, newMilestone) {
      ...
      for (m = 0; m < tlThis.timeline[index]["milestones"].length; m++) {
        if (
          tlThis.timeline[index]["milestones"][m]["startTime"] ==
            newMilestone["startTime"] &&
          tlThis.timeline[index]["milestones"][m]["stopTime"] ==
            newMilestone["stopTime"]
        ) {
          //alert("Dup!")
          dup = true;
          //Todo Test Dups
          if ("annotationID" in newMilestone) {
            tlThis.timeline[index]["milestones"][m]["annotationID"] =
